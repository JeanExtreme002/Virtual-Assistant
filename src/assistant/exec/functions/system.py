...
def open_directory(directory):
    for path in paths.split(";"):
        ...
        if dir_exists(abs_path): return os.startfile(abs_path)
...
def open_file(filename, only_ext = []):
    ...
    for path in paths.split(";"):
        if file_ext and not only_ext:
            ...
            if file_exists(abs_path): return Thread(target = lambda: os.startfile(abs_path)).start()
        else:
            found = get_filename_by_name(filename, os.path.abspath(path), only_ext)
            if found: return Thread(target = lambda: os.startfile(found)).start()
...
def write(text):
    ...
    filename = os.path.join(directory.name, "Speech Text.txt")
